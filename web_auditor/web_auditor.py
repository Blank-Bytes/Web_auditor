...
# web_auditor Help Context
def helper():
        print(bcolors.OKBLUE+"Information:"+bcolors.ENDC)
        print("------------")
        print("\t./web-auditor.py example.com: Scans the domain example.com.")
        print("\t./web-auditor.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./web-auditor.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./web-auditor.py --update   : Updates the scanner to the latest version.")
        print("\t./web-auditor.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits web-auditor.")
...
# web-auditor Logo
def logo():
	ascii_banner = pyfiglet.figlet_format("Web Auditor")
	print(ascii_banner)
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"web-auditor received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/web-auditor_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/web-auditor_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/web-auditor_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/web-auditor_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/web-auditor_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o web-auditor_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("web-auditor is updating....Please wait.\n")
    spinner.start()
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum web-auditor.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N https://raw.githubusercontent.com/skavngr/web-auditor/master/web-auditor.py -O web-auditor.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of web-auditor." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"web-auditor successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    os.system('rm /tmp/web-auditor* > /dev/null 2>&1') # Clearing previous scan files
    os.system('clear')
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"web-auditor was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by web-auditor."+bcolors.ENDC)
    else:
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/web-auditor_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit web-auditor.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    debuglog = "we.dbg.%s.%s" % (target, date) 
    vulreport = "we.vul.%s.%s" % (target, date)
    ...
    if len(rs_vul_list)==0:
        ...
        with open(vulreport, "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/web-auditor_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory web-auditor resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open(debuglog, "a") as report:
            try:
                with open("/tmp/web-auditor_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
...
# Data for the pie chart (using previously processed tool_vul_counts)
    tool_vul_counts = {}
    for item in rs_vul_list:
    	tool_name = item.split('*')[0]  # Extract tool name
    	tool_vul_counts[tool_name] = tool_vul_counts.get(tool_name, 0) + 1  # Increment count
    ...
    with PdfPages('report.pdf') as pdf:
	    ...
	    pdf.savefig()  # Save the bar chart to PDF
	    ...
	    plt.axis('equal')
	    pdf.savefig()  # Save the pie chart to PDF
	    plt.close()  # Close the pie chart figure
	    

    
   # Total number of vulnerabilities detected
    total_vulnerabilities = len(rs_vul_list)

    # Create a PDF file for the vulnerability report
    with PdfPages('vulnerability_report.pdf') as pdf:
        # Select a random subset of vulnerabilities
        selected_vulnerabilities = random.sample(tools_fix, k=total_vulnerabilities) 
        
        # Write vulnerability information to the PDF
        for tool_id, vul_description, vul_remediation in selected_vulnerabilities:
            plt.figure(figsize=(8, 6))
            plt.text(0.5, 0.5, f"Remediation Steps:\n{vul_remediation}",
                     ha='center', va='center', wrap=True, fontsize=12)
            plt.axis('off')
            pdf.savefig()
            plt.close()
            
    #Merging the two Report pdf files to one
    import PyPDF2

    # Open the two PDF files
    with open('report.pdf', 'rb') as report_pdf_file, open('vulnerability_report.pdf', 'rb') as vulnerability_report_pdf_file:
        # Create PDF reader objects
        report_pdf_reader = PyPDF2.PdfReader(report_pdf_file)
        vulnerability_report_pdf_reader = PyPDF2.PdfReader(vulnerability_report_pdf_file)

        # Create a PDF writer object
        combined_pdf_writer = PyPDF2.PdfWriter()

        # Add pages from the report PDF
        for page_num in range(len(report_pdf_reader.pages)):
            page = report_pdf_reader.pages[page_num]
            combined_pdf_writer.add_page(page)

        # Add pages from the vulnerability report PDF
        for page_num in range(len(vulnerability_report_pdf_reader.pages)):
            page = vulnerability_report_pdf_reader.pages[page_num]
            combined_pdf_writer.add_page(page)

        # Write the combined PDF to a new file
        with open('final_report.pdf', 'wb') as combined_pdf_file:
            combined_pdf_writer.write(combined_pdf_file)

    os.system('setterm -cursor on')
    os.system('rm /tmp/web-auditor_te* > /dev/null 2>&1') # Clearing previous scan files
